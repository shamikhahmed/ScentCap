export function timeGreeting(): { period: string; line: string; emoji: string } {
  const h = new Date().getHours();
  if (h < 5) return { period: 'night', line: 'Still awake?', emoji: '🌙' };
  if (h < 12) return { period: 'morning', line: 'Good morning', emoji: '☀️' };
  if (h < 17) return { period: 'afternoon', line: 'Good afternoon', emoji: '🌤️' };
  if (h < 21) return { period: 'evening', line: 'Good evening', emoji: '✨' };
  return { period: 'night', line: 'Good night', emoji: '🌙' };
}
